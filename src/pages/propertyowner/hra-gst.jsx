...
export default function HraGst() {
  const owner = getOwnerRuntimeSession();
  if (!owner?.loginId && typeof window !== "undefined") { window.location.href = "/propertyowner/ownerlogin"; return null; }
  return (
    <PropertyOwnerLayout owner={owner} title="HRA & GST" onLogout={() => { clearOwnerRuntimeSession(); window.location.href = "/propertyowner/ownerlogin"; }}>
      <div className="mb-8"><h1 className="font-serif text-[38px] md:text-[44px] leading-[1.05] text-foreground">HRA & GST</h1>
      <p className="mt-1.5 text-[13.5px] text-muted-foreground">Tax reports and receipts.</p></div>
