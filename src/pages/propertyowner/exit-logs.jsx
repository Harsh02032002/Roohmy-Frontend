...
export default function ExitLogsPage() {
  ...
  return (
    <PropertyOwnerLayout 
      owner={owner} 
      title="Gate Exit Logs" 
      onLogout={() => { clearOwnerRuntimeSession(); window.location.href = "/propertyowner/ownerlogin"; }}
    >
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
        <div>
          <h1 className="font-serif text-[38px] md:text-[44px] leading-[1.05] text-foreground">Gate Exit Logs</h1>
          <p className="mt-1.5 text-[13.5px] text-muted-foreground">Monitor real-time gate logouts, biometric scans, and RFID departures logs.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search exits by name or RFID reference ID..."
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="text-left text-[11.5px] uppercase tracking-wider text-muted-foreground bg-muted/50">
                ...
                <th className="px-6 py-3.5 font-semibold">Check-Out Timestamp</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {loading ? (
                <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">Loading exit logs...</td></tr>
              ) : filteredLogs.length === 0 ? (
                <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">No exit logs found.</td></tr>
              ) : filteredLogs.map((l) => (
