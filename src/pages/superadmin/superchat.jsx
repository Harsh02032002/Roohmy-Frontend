...
export default function SuperChat() {
  ...
  const loadInbox = async () => {
    ...
    setError("");
    try {
      const data = await fetchJson(`/api/chat/inbox/${encodeURIComponent(superadminLoginId)}?search=${encodeURIComponent(search)}`);
      const conversations = Array.isArray(data?.conversations) ? data.conversations : [];
      // Only show website users to match previous behavior
      const websiteUsers = conversations.filter((item) => /^roomhyweb\d{6}$/i.test(String(item?.participant_login_id || "")));
      setInbox(websiteUsers);
      setActiveChat((prev) => {
        if (prev) return websiteUsers.find((x) => x.participant_login_id === prev.participant_login_id) || websiteUsers[0] || null;
        return websiteUsers[0] || null;
      });
    } catch (err) {
  ...
  useEffect(() => { loadInbox(); }, [superadminLoginId, search]);

  useEffect(() => {
    const loadConversation = async () => {
      if (!activeChat?.participant_login_id) return setMessages([]);
      setLoadingMessages(true);
      try {
        const list = await fetchJson(`/api/chat/conversation?user1=${encodeURIComponent(superadminLoginId)}&user2=${encodeURIComponent(activeChat.participant_login_id)}`);
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <div className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1">
            <span>Chat Management</span><ChevronRight size={12} className="opacity-50" /><span className="text-blue-600">Overview</span>
          </div>
        </div>
        <div className="flex items-center gap-3 bg-white border border-slate-100 px-4 py-2 rounded-xl shadow-sm text-xs font-bold text-slate-600">
          <Calendar className="w-4 h-4 text-slate-400" /><span>May 22 - May 28, 2024</span>
        </div>
      </div>
        ...
        <div className="col-span-12 lg:col-span-6 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto bg-slate-50/30 space-y-6">
            ...
            {!loadingMessages && messages.map((msg) => {
              const mine = String(msg.sender_login_id || "").toUpperCase() === superadminLoginId;
              const isImage = msg.message_type === 'image';
              const isFile = msg.message_type === 'file';

              return (
                <div key={msg.key} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                  <div className={cn("max-w-[75%] p-4 rounded-2xl shadow-sm", mine ? "bg-blue-600 text-white rounded-tr-none shadow-lg shadow-blue-100" : "bg-white border border-slate-100 rounded-tl-none")}>
                    {isImage ? (
                      <div className="space-y-2">
                        ...
                        <p className={cn("text-[10px] font-medium opacity-80", mine ? "text-white" : "text-slate-500")}>{msg.text}</p>
                      </div>
                    ) : isFile ? (
                      <a href={msg.file_url} target="_blank" rel="noopener noreferrer" className={cn("flex items-center gap-3 p-3 rounded-xl border transition-all", mine ? "bg-white/10 border-white/20 hover:bg-white/20" : "bg-slate-50 border-slate-100 hover:bg-slate-100")}>
                        <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center shrink-0", mine ? "bg-white/20 text-white" : "bg-blue-50 text-blue-600")}>
                           <Paperclip size={20} />
                        </div>
                        <div className="min-w-0">
                           <p className={cn("text-[11px] font-bold truncate", mine ? "text-white" : "text-slate-800")}>{msg.text.replace('Sent a file: ', '')}</p>
                           <p className={cn("text-[9px] font-medium opacity-60", mine ? "text-white" : "text-slate-500")}>Click to download</p>
                        </div>
                      </a>
                    ) : (
                      <p className={cn("text-xs font-medium leading-relaxed", mine ? "text-white" : "text-slate-600")}>{msg.text}</p>
                    )}
                    <span className={cn("text-[9px] font-bold mt-2 block", mine ? "text-blue-200 text-right" : "text-slate-300")}>{formatClock(msg.createdAt)}</span>
                  </div>
                </div>
              );
            })}
