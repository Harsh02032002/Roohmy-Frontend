...
export default function EmployeeOwner() {
  ...
  const handleAddOwner = async (e) => {
    ...
    setSaving(true);
    try {
      await fetchJson("/api/owners/request", {
        method: "POST",
      ...
      alert("Property Owner request submitted successfully! Superadmin will generate credentials.");
      setSearchParams({ view: "list" });
      loadOwners();
    } catch (err) { alert(err.message || "Failed to request owner"); }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
         <div className="flex flex-col gap-2">
            <h1 className="text-4xl font-bold text-slate-800 tracking-tight leading-none">
              {currentView === "add" ? "Add Property Owner Request" : "Property Owners"}
            </h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">
              {currentView === "add" ? "Submit request for new asset partner" : "Stakeholder Asset Network"}
            </p>
         </div>
         <div className="flex items-center gap-4">
            ...
            >
               ...
               {currentView === "add" ? "Back to Owners List" : "Add Owner Request"}
            </button>
         </div>
      </div>
      ...
      {currentView === "add" ? (
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-500">
           <div className="p-10 border-b border-slate-50 bg-slate-50/50 flex items-center gap-6">
              ...
              <div>
                 <h3 className="text-2xl font-bold text-slate-800 tracking-tight">Property Owner Request</h3>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Submit primary details to Superadmin for credential generation</p>
              </div>
           </div>
           ...
           <form onSubmit={handleAddOwner} className="p-12 space-y-12">
              ...
              <div className="bg-slate-900 rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center justify-between gap-8">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Generated Request ID</p>
                       <div className="flex items-center gap-4">
                          <code className="text-xl font-black text-white tracking-widest">{formLoginId}</code>
                          <span className="w-1 h-1 rounded-full bg-white/20" />
                          <code className="text-sm font-bold text-slate-400">Credentials generated upon admin approval</code>
                       </div>
                    </div>
                 </div>
                 <button type="button" onClick={generateCreds} className="px-6 py-3 bg-white/10 text-white rounded-xl text-[9px] font-bold uppercase tracking-widest hover:bg-white/20 transition-all border border-white/10">Re-generate ID</button>
              </div>
              ...
              <div className="flex justify-end gap-4 pt-4">
                 ...
                 >
                    {saving ? "Submitting Request..." : "Submit to Admin"}
              ...
              <table className="w-full text-left">
                 <thead>
                    <tr className="bg-slate-50/50 text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] border-b border-slate-100">
                       ...
                       <th className="px-6 py-8 text-center">Banking Intel</th>
                        ...
                        <tr key={i} className="group hover:bg-slate-50/50 transition-all duration-300 cursor-pointer" onClick={() => setSelectedOwner(o)}>
                           ...
                           <td className="px-6 py-8 text-center">
                              <span className={cn(
                                 ...
                                 status === "requested" ? "bg-blue-50 text-blue-600 border-blue-100" :
